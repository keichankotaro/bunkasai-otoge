...
public class L4 : MonoBehaviour
{
    ...
    private async Task destroy_gameobject()
    {
        ...
        {
            // 次のノートの種類をJSONから取得
            JObject nextNoteData = JObject.Parse(adata.jsonObj["chartdata"]["L4"][newId + ""] + "");
    ...
    private async Task Initialize()
    {
        game_time = 0.0f;
        id = getName(0);
        //Debug.Log("L4 id: " + id + " Name: " + name);
        cd = JObject.Parse(jsonObj["chartdata"]["L4"][id+""] + "");
            ...
            if (adata.ready_to_start && !EndTask)
            {
                ...
                if (type == "tap")
                {
                    ...
                    if (isPreviousObjectDestroyed)
                    {
                        if (!adata.auto_play)
                        {
                            if (Input.GetKeyDown("k") || Input.GetKeyDown(adata.ControlerLine4))
                            {
                                if (timelag <= adata.perfect)
                                {
                                    // p[tFNg
                                    ////Debug.Log("Prefect");
                                    GameObject h = Instantiate(adata.p, new UnityEngine.Vector3(10.64f, 0.2f, -11.9f), UnityEngine.Quaternion.Euler(68.85f, 0.0f, 0.0f));
                                    h.GetComponent<goodbye>().SetInstantiatorName(name.Split("_")[1] + "", 4);
                                    await Task.Run(() =>
                                    {
                                        ScoreManeger.combo++;
                                        ...
                                        adata.clicked_L4++;
                                        DebugText.judged_l4++;
                                        DebugText.score_l4 += 5;
                                        DebugText.perfect_l4++;
                                    });
                                    EndTask = true;
                                ...
                                else if (timelag <= adata.good)
                                {
                                    // Obh
                                    ////Debug.Log("Good");
                                    GameObject h = Instantiate(adata.g, new UnityEngine.Vector3(10.74f, 0.2f, -11.9f), UnityEngine.Quaternion.Euler(68.85f, 0.0f, 0.0f));
                                    h.GetComponent<goodbye>().SetInstantiatorName(name.Split("_")[1] + "", 4);
                                    await Task.Run(() =>
                                    {
                                        ScoreManeger.combo++;
                                ...
                                else if (timelag <= adata.bad)
                                {
                                    // obh
                                    ////Debug.Log("Bad");
                                    GameObject h = Instantiate(adata.b, new UnityEngine.Vector3(10.77f, 0.2f, -11.9f), UnityEngine.Quaternion.Euler(68.85f, 0.0f, 0.0f));
                                    h.GetComponent<goodbye>().SetInstantiatorName(name.Split("_")[1] + "", 4);
                                    await Task.Run(() =>
                                    {
                            ...
                            if (game_time > arrsec + adata.miss)
                            {
                                // ~X
                                ////Debug.Log("Miss");
                                GameObject h = Instantiate(adata.m, new UnityEngine.Vector3(10.78f, 0.2f, -11.9f), UnityEngine.Quaternion.Euler(68.85f, 0.0f, 0.0f));
                                h.GetComponent<goodbye>().SetInstantiatorName(name.Split("_")[1] + "", 4);
                                await Task.Run(() =>
                                {
                                    ScoreManeger.combo = 0;
                ...
                if (isLongNoteActive)
                {
                    ...
                    if (isPreviousObjectDestroyed)
                    {
                        ...
                        {
                            if ((Input.GetKeyDown("k") || Input.GetKeyDown(adata.ControlerLine4)) && !long_click)
                            {
                                if (timelag <= adata.perfect)
                                {
                                    // p[tFNg
